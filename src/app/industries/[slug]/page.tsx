          ...
          {industry.workflowSteps.map((step, index, list) => (
            ...
            >
              <p className="tnum flex items-center gap-2.5 text-[13px] font-medium text-ink/60">
                <span aria-hidden="true" className="inline-block size-[7px] bg-cobalt" />0
                {index + 1}
              </p>
              <h2 className="mt-4 text-[18px] leading-[1.4] font-semibold md:mt-6 md:text-[19px]">
                {step.title}
              </h2>
              <p className="mt-3 text-[14px] leading-relaxed text-ink/70">{step.description}</p>
              {index < list.length - 1 && (
                <span
                  aria-hidden="true"
        ...
        <div className="grid lg:grid-cols-[1fr_500px]">
          <div className="border-b border-hairline px-5 py-8 md:px-10 md:py-12 lg:border-r lg:border-b-0">
            ...
            <p className="tnum mt-4 max-w-[52ch] text-[16px] leading-relaxed text-ink/80">
              <span className="font-semibold text-ink">10 to 14 weeks</span> from kickoff to a live
              production system. We stay through adoption.
            </p>
          </div>
            ...
            {INDUSTRY_DELIVERABLES.map((deliverable, index) => (
              <div
                ...
                className={`flex grow items-baseline gap-4 px-5 py-5 md:px-8 ${
                  index < INDUSTRY_DELIVERABLES.length - 1 ? 'border-b border-hairline' : ''
                }`}
              >
                ...
                <div>
                  <p className="tnum text-[14px] font-semibold">{deliverable.name}</p>
                  <p className="tnum mt-0.5 text-[13px] text-ink/70">{deliverable.detail}</p>
                </div>
              </div>
            ))}
          </div>
