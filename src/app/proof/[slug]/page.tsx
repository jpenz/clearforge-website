...
export default async function CaseStudyPage({ params }: PageProps<'/proof/[slug]'>) {
  const { slug } = await params;
  const study = getCaseStudy(slug);
  if (!study) notFound();
    ...
    <>
      ...
      <PageFrame aria-label="Case study title">
        ...
        <div className="px-5 pt-10 pb-10 md:px-10 md:pt-16 md:pb-14">
          <h1 className="font-display tnum max-w-[24ch] text-[clamp(38px,4.2vw,76px)] leading-[1.05] font-medium tracking-[-0.01em]">
            {study.headline} <em className="text-cobalt italic">{study.headlineEmphasis}</em>
          </h1>
          <p className="mt-6 max-w-[62ch] text-[16px] leading-relaxed text-ink/80">{study.intro}</p>
        </div>
      </PageFrame>
        ...
        >
          {study.glance.map((figure) => (
            <div key={figure.label} className="px-5 py-8 md:px-10 md:py-12">
              ...
              <p className="tnum mt-4 text-[13px] leading-relaxed text-ink/70">{figure.label}</p>
            </div>
          ))}
        </div>
      </PageFrame>
        ...
        <PageFrame key={section.label} aria-label={section.label}>
          <div className="grid lg:grid-cols-[320px_1fr]">
            <div className="border-b border-hairline px-5 py-8 md:px-10 md:py-12 lg:border-r lg:border-b-0">
              ...
              <p className="tnum mt-3 text-[13px] text-ink/70">{section.figureLabel}</p>
            </div>
            <div>
              ...
              {section.label === 'Results' && study.resultsTable && (
                <table className="w-full border-t border-hairline text-left">
                  <caption className="sr-only">Case {study.letter} results by metric</caption>
                  <thead>
                  ...
                  <tbody>
                    {study.resultsTable.map((row) => (
                      ...
                      >
                        <td className="px-5 py-4 text-[14px] text-ink/80 md:px-10">{row.metric}</td>
                        <td
