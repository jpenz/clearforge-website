...
describe('QuestionCard', () => {
  ...
  it('shows the question number prefix', () => {
    render(
      <QuestionCard question={mockQuestion} value={undefined} onChange={vi.fn()} index={0} />,
    );
  ...
  it('does not crash when value is 0', () => {
    expect(() =>
      render(
        <QuestionCard question={mockQuestion} value={0} onChange={vi.fn()} index={0} />,
      ),
    ).not.toThrow();
  });

  it('renders correctly for the last question (index 17)', () => {
    const lastQuestion: ScorecardQuestion = { id: 18, text: 'Strategy question', pillar: 'strategy' };
    expect(() =>
      render(
        <QuestionCard question={lastQuestion} value={4} onChange={vi.fn()} index={17} />,
      ),
    ).not.toThrow();
  });
});
