...
import { type FormState, unlockScoreReport } from '@/app/actions';
...
export function ScoreResults({ result, onRetake }: ScoreResultsProps) {
  const [state, formAction, pending] = useActionState(unlockScoreReport, INITIAL);
        ...
        <div className="flex flex-col border-b border-hairline lg:border-r lg:border-b-0">
          ...
          <div className="flex grow flex-col px-4 pt-8 pb-8 md:px-8 md:pt-10">
            <p className="tnum leading-none">
              ...
              <span className="text-[32px] font-light text-ink/60 md:text-[40px]">/100</span>
            </p>
          ...
          <div className="flex grow flex-col divide-y divide-hairline">
            {PILLARS.map((pillar, index) => (
              ...
              >
                ...
                <span aria-hidden="true" className="relative h-px grow bg-hairline">
                  <span
          ...
          {state.status === 'success' ? (
            ...
            <>
              <p className="text-[16px] leading-relaxed text-ink/80">
                The detailed readout explains each pillar score. Unlock it with your work details.
              </p>
              <form
                action={formAction}
              ...
              >
                ...
                <input
                  ...
                  name="pillars"
                  value={PILLARS.map(
                    (pillar, index) => `${pillar.name}: ${result.pillarScores[index]}`,
                  ).join(', ')}
                />
              ...
              {state.status === 'error' && (
                <p role="alert" className="mt-4 border-l-2 border-ink pl-3 text-[13px] text-ink/80">
                  {state.message}
                </p>
              )}
