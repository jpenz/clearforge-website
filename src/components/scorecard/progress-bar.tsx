...
export function ProgressBar({ currentStep, completedSteps }: ProgressBarProps) {
  ...
  return (
    <div className="mb-10">
      ...
      {/* Progress bar */}
      <div role="progressbar" aria-valuenow={Math.round(progress)} aria-valuemin={0} aria-valuemax={100} className="h-1 w-full overflow-hidden bg-divider">
        <div className="h-full bg-brass transition-all duration-500 ease-out" style={{ width: `${progress}%` }} />
      </div>
      ...
      <div className="mt-3 flex items-center justify-between gap-4">
        ...
        <p className="metric text-xs text-brass shrink-0">{currentStep + 1} / {pillars.length}</p>
      </div>
    </div>
  );
}
