    ...
    <motion.div className="flex flex-col items-center" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }}>
      <div className="relative flex h-32 w-32 items-center justify-center sm:h-40 sm:w-40">
        <svg className="absolute inset-0 -rotate-90" viewBox="0 0 160 160">
          <circle cx="80" cy="80" r="70" fill="none" stroke="currentColor" strokeWidth="8" className="text-fog" />
          <circle cx="80" cy="80" r="70" fill="none" stroke="currentColor" strokeWidth="8" strokeDasharray={`${(animatedScore / 100) * 440} 440`} strokeLinecap="round" className="text-brass transition-all duration-1000" />
        </svg>
        <span className="metric-display text-5xl text-brass">{animatedScore}</span>
      </div>
      <p className="mt-2 text-xs uppercase tracking-widest text-stone">out of 100</p>
    </motion.div>
  );
}
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay }}>
      <div className="mb-1 flex items-center justify-between text-sm">
        ...
        <span className="metric-display font-medium text-midnight">{Math.round(percentage)}%</span>
      </div>
      <div className="h-1.5 w-full overflow-hidden bg-fog">
        <motion.div className="h-full bg-brass" initial={{ width: 0 }} animate={{ width: `${percentage}%` }} transition={{ duration: 0.8, delay: delay + 0.2 }} />
      </div>
    </motion.div>
  );
}
...
export function ResultsDisplay() {
  ...
  if (!results) {
    return <div className="flex min-h-[60vh] items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-fog border-t-brass" /></div>;
  }
    ...
    <div className="bg-white py-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto mt-12 max-w-4xl">
          <div className="grid gap-0 overflow-hidden border border-fog md:grid-cols-2">
            <motion.div className="flex flex-col items-center border-b border-fog bg-ivory p-8 md:border-b-0 md:border-r" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
              <ScoreGauge score={results.compositeScore} />
              <div className="mt-6 text-center">
                <p className="text-xs uppercase tracking-widest text-stone">Maturity Level</p>
                <p className="mt-1 text-2xl" style={{ fontFamily: "var(--font-bodoni)" }}>{results.maturityLevel}</p>
              </div>
            </motion.div>

            <motion.div className="flex flex-col bg-ivory p-8" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }}>
              <h3 className="text-lg" style={{ fontFamily: "var(--font-bodoni)" }}>What This Means</h3>
              <p className="mt-3 flex-1 text-sm text-stone leading-relaxed">{results.maturityDescription}</p>
              <div className="mt-4 border-l-2 border-brass bg-brass/5 p-4 pl-5">
                <p className="section-label">Our Recommendation</p>
                <p className="mt-1 text-sm text-stone">{results.recommendation}</p>
              </div>
            </motion.div>
          </div>
        </div>

        <motion.div className="mx-auto mt-12 max-w-4xl border border-fog bg-ivory p-6 sm:p-8" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.3 }}>
        ...
        {isGated && (
          <motion.div className="mx-auto mt-12 max-w-4xl" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.5 }}>
            <div className="border border-brass/20 bg-brass/5 p-8 text-center">
              <Lock className="mx-auto h-8 w-8 text-brass" />
              <h3 className="mt-4 text-xl" style={{ fontFamily: "var(--font-bodoni)" }}>Get Your Full Report</h3>
              <p className="mx-auto mt-2 max-w-md text-sm text-stone">Enter your email to receive a detailed breakdown with personalized recommendations.</p>
              <form onSubmit={handleEmailSubmit} className="mx-auto mt-6 flex max-w-md flex-col gap-3 sm:flex-row">
                <Input type="email" placeholder="your@email.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="flex-1 border-fog bg-white text-midnight placeholder:text-stone/50" />
        ...
        {submitted && !isGated && (
          <motion.div className="mx-auto mt-12 max-w-4xl" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <div className="border border-forest/30 bg-forest/5 p-6">
              <p className="text-sm font-medium text-forest">Report sent to {email}</p>
              <p className="mt-1 text-sm text-stone">Check your inbox for your personalized AI readiness report.</p>
            </div>
          </motion.div>
        )}
        ...
        <motion.div className="mt-24 text-center" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.4 }}>
          ...
          <p className="mx-auto mt-4 max-w-xl text-stone">Our recommended next step: <span className="font-semibold text-midnight">{results.recommendedService}</span></p>
