...
export function Problem() {
  return (
    <section className="bg-navy py-20 lg:py-28 grain-overlay section-divider">
            ...
            >
              ...
              <div className="mb-5 mt-4 flex items-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-lg border border-white/10 bg-white/5">
                  <problem.icon className="h-5 w-5 text-white/70" />
                </div>
                <span className="font-[family-name:var(--font-heading)] text-3xl font-bold text-white/20">
                  {problem.number}
                </span>
              </div>

              <h3 className="text-2xl font-bold leading-tight text-white" style={{ fontFamily: "var(--font-heading)" }}>
                {problem.title}
              </h3>

              <p className="mt-3 text-base leading-relaxed text-white/70">{problem.description}</p>

              {i < 2 && (
                <div className="absolute -right-3 top-1/2 z-10 hidden h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-navy md:flex">
