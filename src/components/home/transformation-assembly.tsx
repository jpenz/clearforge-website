...
const FUNCS = [
  { label: "Sales",      x: 460, y: 72,  w: 110, h: 40, hotspot: true,  metric: "+41%", mLabel: "Revenue" },
  { label: "Marketing",  x: 660, y: 180, w: 118, h: 40, hotspot: true,  metric: "3.2x", mLabel: "Pipeline" },
  { label: "Operations", x: 380, y: 390, w: 126, h: 40, hotspot: true,  metric: "97%",  mLabel: "Efficiency" },
...
const BENEFITS = [
  { x: (FUNCS[0].x + AGENTS[0].x) / 2 + 8, y: (FUNCS[0].y + AGENTS[0].y) / 2 - 16, label: "AUTOMATED OUTREACH" },
  { x: (FUNCS[1].x + AGENTS[1].x) / 2, y: (FUNCS[1].y + AGENTS[1].y) / 2 - 16, label: "AI-DRIVEN TARGETING" },
  { x: (FUNCS[2].x + AGENTS[2].x) / 2, y: (FUNCS[2].y + AGENTS[2].y) / 2 - 16, label: "WORKFLOW AUTOMATION" },
];
...
const AMBIENT = [
  { cx: 120, cy: 60 }, { cx: 720, cy: 90 }, { cx: 80, cy: 200 },
  { cx: 750, cy: 320 }, { cx: 150, cy: 440 }, { cx: 680, cy: 420 },
  { cx: 50, cy: 340 }, { cx: 600, cy: 50 }, { cx: 90, cy: 140 },
  { cx: 560, cy: 450 }, { cx: 340, cy: 30 }, { cx: 480, cy: 460 },
];
...
function ForgeAnimation({ progressRef }: { progressRef: React.RefObject<{ value: number } | null> }) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (dashGlow) {
        dashGlow.style.opacity = phase < 1 ? "0" : phase === 1 ? String(ease(cl((t - 0.3) / 0.3)) * 0.04) : "0.06";
      }
      ...
      if (grid) {
        grid.style.opacity = phase === 0 ? String(ease(cl((t - 0.1) / 0.3)) * 0.08) : phase === 1 ? "0.06" : "0.04";
      }
    ...
    >
      ...
      {FUNCS.map((f, i) => (
        <g key={`fn${i}`} className="fg-func" data-x={f.x} style={{ opacity: 0 }}>
          <rect
            ...
            width={f.w} height={f.h} rx="6"
            fill={`${BG}cc`} stroke={TEAL} strokeWidth="1.5"
          />
          <text
            x={f.x} y={f.y + (f.hotspot ? -2 : 5)}
            textAnchor="middle" fill={TEAL}
      ...
      {hotspotFuncs.map((f, i) => (
        <g key={`hs${i}`} className="fg-hotspot" data-x={f.x} style={{ opacity: 0 }}>
          <circle cx={f.x} cy={f.y} r={f.w / 2 + 12} fill="none" stroke={TEAL} strokeWidth="1.5" filter="url(#fgGlow)" />
          <circle cx={f.x} cy={f.y} r={f.w / 2 + 20} fill="none" stroke={TEAL} strokeWidth="0.5" strokeDasharray="4 6" />
        </g>
      ))}
      ...
      {SCAN_TAGS.map((group, gi) => {
        ...
        return group.tags.map((tag, ti) => {
          ...
          return (
            <g key={`st${gi}${ti}`} className="fg-scan-tag" data-x={String(group.fx)} data-delay={String(ti * 0.18)} style={{ opacity: 0 }}>
              <rect
                x={f.x + tag.dx - tw / 2} y={f.y + tag.dy - 7}
              ...
              <rect
                x={f.x + tag.dx - tw / 2} y={f.y + tag.dy - 7}
                width={tw} height="14" rx="3"
                fill="none" stroke={TEAL} strokeWidth="0.5" opacity="0.35"
              />
              <text
                x={f.x + tag.dx} y={f.y + tag.dy + 3}
                textAnchor="middle" fill={TEAL}
      ...
      {AGENTS.map((a, i) => (
        <g key={`ag${i}`} className="fg-agent" style={{ opacity: 0 }}>
          <circle cx={a.x} cy={a.y} r="22" fill="none" stroke={TEAL} strokeWidth="0.5" opacity="0.2" />
          ...
          <circle cx={a.x + 3.5} cy={a.y + 2.5} r="1.4" fill={TEAL} opacity="0.85" />
          <line x1={a.x} y1={a.y - 3.5} x2={a.x - 3.5} y2={a.y + 2.5} stroke={TEAL} strokeWidth="0.5" opacity="0.5" />
          <line x1={a.x} y1={a.y - 3.5} x2={a.x + 3.5} y2={a.y + 2.5} stroke={TEAL} strokeWidth="0.5" opacity="0.5" />
          <line x1={a.x - 3.5} y1={a.y + 2.5} x2={a.x + 3.5} y2={a.y + 2.5} stroke={TEAL} strokeWidth="0.5" opacity="0.5" />
          <text
            x={a.x} y={a.y + 10}
            textAnchor="middle" fill={TEAL}
            fontSize="5" fontWeight="700" fontFamily="system-ui"
      ...
      <g className="fg-dash" style={{ opacity: 0 }}>
        <rect
          x={DASH.x} y={DASH.y}
          width={DASH.w} height={DASH.h} rx="7"
          fill={`${BG}dd`} stroke={TEAL} strokeWidth="1.5"
        />
        <line x1={DASH.x} y1={DASH.y + 20} x2={DASH.x + DASH.w} y2={DASH.y + 20} stroke={TEAL} strokeWidth="0.6" opacity="0.4" />
        <text
          x={DASH.x + DASH.w / 2} y={DASH.y + 14}
          textAnchor="middle" fill={TEAL}
        ...
        <g className="fg-dash-fill" style={{ opacity: 0 }}>
          <rect x={DASH.x + 12} y={DASH.y + 76} width="11" height="16" rx="2" fill={TEAL} opacity="0.25" />
          <rect x={DASH.x + 28} y={DASH.y + 66} width="11" height="26" rx="2" fill={TEAL} opacity="0.4" />
          <rect x={DASH.x + 44} y={DASH.y + 54} width="11" height="38" rx="2" fill={TEAL} opacity="0.55" />
          <rect x={DASH.x + 60} y={DASH.y + 40} width="11" height="52" rx="2" fill={TEAL} opacity="0.7" />
        </g>
        ...
        <g className="fg-dash-fill" style={{ opacity: 0 }}>
          <polyline
            ...
            stroke={TEAL} strokeWidth="1.5" fill="none" opacity="0.75"
          />
          <polygon
        ...
        <g className="fg-dash-fill" style={{ opacity: 0 }}>
          <text
            x={DASH.x + DASH.w / 2} y={DASH.y + DASH.h - 4}
            textAnchor="middle" fill={TEAL}
            fontSize="7" fontWeight="600" fontFamily="monospace" opacity="0.5"
          >
            LIVE · 24/7 ACTIVE
          </text>
        </g>
      ...
      {MARKET_PATHS.map((path, i) => (
        <circle key={`mk${i}`} className="fg-market" r="2" fill={TEAL} filter="url(#fgGlow)" style={{ opacity: 0 }}>
      ...
      {FLOW_PATHS.map((path, i) => (
        <circle key={`fl${i}`} className="fg-flow" r="2.5" fill={TEAL} filter="url(#fgGlow)" style={{ opacity: 0 }}>
...
export function TransformationAssembly({ phases }: TransformationAssemblyProps) {
  ...
  useGSAP(
    () => {
      ...
      tl.to(cards[1], { opacity: 1, y: 0, duration: 0.06 }, 0.30);
      tl.to(cards[0], { opacity: 0.3, duration: 0.04 }, 0.30);
  ...
  return (
    <div ref={containerRef} className="bg-bg-deep">
      <div className="relative h-screen w-full overflow-hidden">
        ...
        <div
          className="relative z-10 flex flex-col justify-center h-full w-full lg:w-[55%] px-6 sm:px-10 lg:px-16 xl:px-20 py-16 lg:py-0"
        >
          ...
          <div className="mt-10 space-y-5">
            {phases.map((phase, i) => (
              <div
                key={phase.num}
                className="ta-card"
                style={{ opacity: i === 0 ? 1 : 0.18, transform: i === 0 ? "none" : "translateY(6px)" }}
              >
                <div
                  className="border-l-[2px] pl-5 py-2"
                  style={{ borderColor: `${TEAL}35` }}
                >
                  <p
          ...
          <div className="mt-10 flex items-center gap-2 opacity-20">
            ...
            <p className="text-[9px] uppercase tracking-[0.2em] text-text-muted">Scroll to explore</p>
          </div>
        </div>
