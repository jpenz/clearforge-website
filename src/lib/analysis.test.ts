...
async function collect(generator: AsyncGenerator<AnalysisEvent>): Promise<AnalysisEvent[]> {
...
describe('streamAnalysis brief mode', () => {
  it('yields progress, then 4 fields, then done, with the approved build window', async () => {
    const events = await collect(streamAnalysis('an industrial manufacturer', 'brief'));
  ...
  it('routes a services target to the services profile', async () => {
    const events = await collect(streamAnalysis('a commercial services firm', 'brief'));
    const workflow = events.find((event) => event.type === 'field' && event.key === 'workflow');
...
describe('streamAnalysis detailed mode', () => {
  it('yields 4 progress steps, then 4 detailed fields, then done', async () => {
    const events = await collect(streamAnalysis('a commercial services firm', 'detailed'));

    const progress = events.filter((event) => event.type === 'progress');
    expect(progress.map((event) => event.label)).toEqual(DETAILED_PROGRESS_STEPS);

    const fields = events.filter((event) => event.type === 'field');
    expect(fields.map((event) => event.key)).toEqual(['workflow', 'handoffs', 'fits', 'measure']);
