...
const industryMultipliers: Record<string, { labor: number; error: number; productivity: number }> =
  {
...
export function calculateROI(inputs: ROIInputs): ROIResult {
  ...
  const laborSavings = Math.round(hoursAutomated * costPerHour * weeksPerYear * multiplier.labor);
  ...
  const processRevenueFraction = Math.min(manualHoursPerWeek / (employees * 40), 0.3);
  ...
  const productivityGain = Math.round((hoursAutomated / (employees * 40)) * 100);
